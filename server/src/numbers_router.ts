...
numbersRouter.get("/funfacts/:num", async (inRequest, inResponse) => {
  ...
  try {
    ...
    const numberInfo: INumberInfo = await numbersWorker.listFunFacts(inRequest.params.num);
    console.log(`GET /numbers/funfacts/:num - OK. Number choosen =  ${numberInfo._number}`);
...
numbersRouter.get("/", async (inRequest, inResponse) => {
  ...
  try {
    const numbersWorker: NumbersWorker = new NumbersWorker();
    const numberInfo: INumberInfo = await numbersWorker.listInfo(Math.round(Math.random()*10000));
...
numbersRouter.get("/:num", async (inRequest, inResponse) => {
  ...
  try {
    const numbersWorker: NumbersWorker = new NumbersWorker();
    const numberInfo: INumberInfo = await numbersWorker.listInfo(inRequest.params.num);
    console.log(`GET /numbers: OK. Number choosen =  ${numberInfo._number}`);
    inResponse.json(numberInfo);
  } catch (inError) {
    console.log("GET /numbers: Error", inError);
